...
float *prepare_filter(float *B, const mwSize *B_dims) {  
  float *F = (float *)mxCalloc(B_dims[0]*B_dims[1]*B_dims[2], sizeof(float));
...
float *prepare_map(float *A, const mwSize *A_dims) {  
  float *F = (float *)mxCalloc(A_dims[0]*A_dims[1]*A_dims[2], sizeof(float));
...
void *process(void *thread_arg) {
  ...
  for (int x = 0; x < C_dims[1]; x++) {
    for (int y = 0; y < B_dims[0]; y++) {
      float *A_off = A + x*(A_dims[0]*A_dims[2]) + y;
      float *B_off = B + y*(B_dims[1]*B_dims[2]);
      float *C_off = C + x*C_dims[0];
      char chn = 'N';
      float one = 1;
      ...
      ptrdiff_t incx = 1;
      ptrdiff_t incy = 1;
      sgemv(&chn, &m, &n, &one, A_off, &lda, B_off, &incx, &one, C_off, &incy);
    }
  }
  pthread_exit(NULL);
}
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) { 
  if (nrhs != 4) {
    mexErrMsgTxt("fconvblasfloat.cc: Wrong number of inputs"); 
  }
  if (nlhs != 1)
    mexErrMsgTxt("fconvblasfloat.cc: Wrong number of outputs");

  // get A
  const mxArray *mxA = prhs[0];
  if (mxGetNumberOfDimensions(mxA) != 3 || 
      mxGetClassID(mxA) != mxSINGLE_CLASS)
    mexErrMsgTxt("fconvblasfloat.cc: Invalid input: A");
  ...
  if (mxGetClassID(cellB) != mxCELL_CLASS)
    mexErrMsgTxt("fconvblasfloat.cc: Invalid input B, must be a cell array\n");
  ...
  if (start < 0 || end >= num_bs || start > end)
    mexErrMsgTxt("fconvblasfloat.cc Invalid input: start/end");
  int len = end-start+1;
  ...
  float *A = prepare_map((float *)mxGetPr(mxA), A_dims);
  ...
  for (int i = 0; i < len; i++) {
    ...
    if (mxGetNumberOfDimensions(mxB) != 3 ||
      ...
      char buffer[200];
      sprintf(buffer,"fconvblasfloat.cc: Invalid input: B. dim[B]=[%d,%d,%d] dim[A]=[%d,%d,%d]",
    ...
    td[i].C_dims[1] = width;
    td[i].mxC = mxCreateNumericArray(2, td[i].C_dims, mxSINGLE_CLASS, mxREAL);
    td[i].C = (float *)mxGetPr(td[i].mxC);

    if (skips[i] == false)
    {
      if (pthread_create(&ts[i], NULL, process, (void *)&td[i]))
        mexErrMsgTxt("fconvblasfloat.cc: Error creating thread");  
